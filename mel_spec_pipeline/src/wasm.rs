    ...
    pub fn add(&mut self, data: Vec<f32>) -> JsValue {
        ...
        if self.accumulated_samples.len() >= self.hop_size {
            ...
            if let Some(fft) = self.fft.add(&samples.to_vec()) {
                ...
                if let Some((va, gaps)) = self.vad.add(&frame2) {
                    ...
                    let gaps_array = Array::new_with_length(gaps.len() as u32);
                    for (index, value) in gaps.iter().enumerate() {
                        gaps_array.set(index as u32, JsValue::from(*value as i32));
                    }
                    Reflect::set(&result, &JsValue::from_str("gaps"), &gaps_array).unwrap();
                }
            }
